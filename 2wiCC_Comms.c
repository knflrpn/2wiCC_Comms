...
uint8_t data_feedback_1 = 0, data_feedback_2 = 0;
...
int main() {
    ...
    while (1) {
        uint8_t r = data_feedback_1 & 0x3;
        uint8_t g = data_feedback_2 & 0x3;
        uint8_t b = ((++conn_feedback >> 2) % 4) * ((tud_mounted() && tud_ready()) ? 1 : 0);
